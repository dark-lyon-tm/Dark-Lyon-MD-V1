...
async function startDarklyon() {
    const Darklyon = DarklyonConnect({
    ...
    store.bind(Darklyon.ev)
    
    // anticall auto block
    Darklyon.ws.on('CB:call', async (json) => {
    const callerId = json.content[0].attrs['call-creator']
    if (json.content[0].tag == 'offer') {
    let xeonfek = await Darklyon.sendContact(callerId, global.owner)
    Darklyon.sendMessage(callerId, { text: `Automatic Block System!\nDon't Call Bot!\nPlease Ask Or Contact The Owner To Unblock You!`}, { quoted : xeonfek })
    await sleep(8000)
    await Darklyon.updateBlockStatus(callerId, "block")
    }
    })

    Darklyon.ev.on('messages.upsert', async chatUpdate => {
    ...
    // Get Profile Picture Group
       try {
       ppgc = await Darklyon.profilePictureUrl(pea[0].id, 'image')
       } catch {
...
let docs = pickRandom(documents)
    ...
    Darklyon.ev.on('group-participants.update', async (anu) => {
        console.log(anu)
        try {
            let metadata = await Darklyon.groupMetadata(anu.id)
            let participants = anu.participants
            for (let num of participants) {
                // Get Profile Picture User
                try {
                    ppuser = await Darklyon.profilePictureUrl(num, 'image')
                } catch {
                ...
                //Get Profile Picture Group\\
                try {
                    ppgroup = await Darklyon.profilePictureUrl(anu.id, 'image')
                } catch {
...
└┬❖ 「 @${xeonName.split("@")[0]}  」
   ...
   //dont forget to put my name(Darklyon) as credit
...
Darklyon.sendMessage(anu.id, buttonMessage, {quoted:unicorndoc})
...
Darklyon.sendMessage(anu.id, buttonMessage, {quoted:unicorndoc})
    ...
    Darklyon.ev.on('contacts.update', update => {
        for (let contact of update) {
            let id = Darklyon.decodeJid(contact.id)
            if (store && store.contacts) store.contacts[id] = { id, name: contact.notify }
        }
    })

    Darklyon.getName = (jid, withoutContact  = false) => {
        id = Darklyon.decodeJid(jid)
        withoutContact = Darklyon.withoutContact || withoutContact 
        let v
        if (id.endsWith("@g.us")) return new Promise(async (resolve) => {
            v = store.contacts[id] || {}
            if (!(v.name || v.subject)) v = Darklyon.groupMetadata(id) || {}
            resolve(v.name || v.subject || PhoneNumber('+' + id.replace('@s.whatsapp.net', '')).getNumber('international'))
        })
        else v = id === '0@s.whatsapp.net' ? {
            id,
            name: 'WhatsApp'
        } : id === Darklyon.decodeJid(Darklyon.user.id) ?
        Darklyon.user :
            (store.contacts[id] || {})
	...
	for (let i of kon) {
	    list.push({
	    	displayName: await Darklyon.getName(i + '@s.whatsapp.net'),
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await Darklyon.getName(i + '@s.whatsapp.net')}\nFN:${global.ownername}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Click here to chat\nitem2.EMAIL;type=INTERNET:${global.ytname}\nitem2.X-ABLabel:YouTube\nitem3.URL:${global.socialm}\nitem3.X-ABLabel:GitHub\nitem4.ADR:;;${global.location};;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
    ...
    Darklyon.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            ...
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); Darklyon.logout(); }
        ...
        console.log('Dark Lyon is Connected...Broooooo...', update)
    })
     ...
     * @returns 
     */
     Darklyon.sendTextWithMentions = async (jid, text, quoted, options = {}) => XeonBotInc.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
     ...
     */
     Darklyon.sendImageAsSticker = async (jid, path, quoted, options = {}) => {
        ...
        await Darklyon.sendMessage(jid, { sticker: { url: buffer }, ...options }, { quoted })
        return buffer
    }
